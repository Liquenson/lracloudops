...
## REGLAS DE GIT — OBLIGATORIAS EN TODAS LAS SESIONES

### Al terminar CUALQUIER tarea o conjunto de cambios:

1. Ejecuta siempre en este orden:
   ```bash
   git add -A
   git commit -m "tipo: descripción concisa de los cambios"
   git push origin feat/reorder-homepage-sections
   ```

2. Crea PR automáticamente con:
   ```bash
   gh pr create \
     --title "tipo: descripción" \
     --body "## Cambios\n- cambio 1\n- cambio 2\n\nBuild: X páginas, 0 errores" \
     --base main \
     --head feat/reorder-homepage-sections
   ```

3. El auto-merge se encargará del merge cuando CI pase.
...
### NUNCA dejes cambios sin commitear al final de una sesión.

### SIEMPRE verifica build antes del commit:
```bash
npm run build 2>&1 | tail -5
npx prettier --check "src/**/*.{astro,ts,css}" 2>&1 | tail -5
```

### Si Prettier falla, corrígelo antes del commit:
```bash
npx prettier --write "src/**/*.{astro,ts,css}"
```
